...
class ofxVolume_{// : public ofBaseImage_<PixelType>{
	...
	public :
		...
		ofxVolume_(const ofxVoxels_<PixelType> & voxls);
		ofxVolume_(const ofDirectory & folder);
		...
		ofxVolume_(const ofxVolume_<PixelType>& mom);
		ofxVolume_<PixelType>& operator= (const ofxVolume_<PixelType>& mom);

		template<typename SrcType>
		ofxVolume_(const ofxVolume_<SrcType>& mom);

		template<typename SrcType>
		ofxVolume_<PixelType>& operator= (const ofxVolume_<SrcType>& mom);

		// copying:
		template<typename SrcType>
		void 				clone(const ofxVolume_<SrcType> &mom);
	...
	//		void 				saveVolume(ofBuffer & buffer, ofImageQualityType compressionLevel = OF_IMAGE_QUALITY_BEST);
		...
		// getting the data
		PixelType * 				getVoxels();			// up to you to get this right
		ofxVoxels_<PixelType> &	 	getVoxelsRef();
		...
		void 				grabScreen(int x, int y);		// grab voxels from opengl, using glreadvoxels
		...
		void				cropFrom(ofxVolume_<PixelType> & otherImage, int x, int y, int z, int w, int h, int d);
...
*/
		...
		ofxVolume_<PixelType> & operator=(ofxVoxels_<PixelType> & voxels);
	protected:
...
typedef ofxVolume_<unsigned char> ofxVolume;
typedef ofxVolume_<float> ofxFloatVolume;
typedef ofxVolume_<unsigned short> ofxShortVolume;


//----------------------------------------------------------
template<typename PixelType>
template<typename SrcType>
ofxVolume_<PixelType>& ofxVolume_<PixelType>::operator=(const ofxVolume_<SrcType>& mom) {
	clone(mom);
	update();
	return *this;
}
...
template<typename PixelType>
template<typename SrcType>
ofxVolume_<PixelType>::ofxVolume_(const ofxVolume_<SrcType>& mom) {
	clear();
	clone(mom);
	update();
}
...
template<typename PixelType>
template<typename SrcType>
void ofxVolume_<PixelType>::clone(const ofxVolume_<SrcType> &mom){
	ofxVolume_<SrcType> & nonConst = const_cast<ofxVolume_<SrcType> & >(mom);
	voxels = nonConst.getVoxelsRef();
